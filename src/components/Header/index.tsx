...
import { mainNavItems, authActions, logo, primaryActions } from './config'
              ...
              <Popover key={item.name} className="relative">
                ...
                >
                    ...
                    {item.items?.map((subItem) => (
                      ...
                      >
                        <div className="flex size-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                          <subItem.icon aria-hidden="true" className="size-6 text-gray-600 group-hover:text-indigo-600" />
                        </div>
                  ...
                  <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
                    {item.callsToAction?.map((action) => (
                      <a
                        key={action.name}
                        href={action.href}
                        className="flex items-center justify-center gap-x-2.5 p-3 text-sm/6 font-semibold text-gray-900 hover:bg-gray-100"
                      >
                        {action.icon && <action.icon aria-hidden="true" className="size-5 flex-none text-gray-400" />}
                        {action.name}
                      </a>
                    ))}
                  </div>
                  ...
                  item.type === 'dropdown' ? (
                    <Disclosure key={item.name} as="div" className="-mx-3">
                      ...
                      <DisclosurePanel className="mt-2 space-y-2">
                        {[...(item.items || []), ...(item.callsToAction || [])].map((subItem) => (
                          <DisclosureButton
                            key={subItem.name}
                            as="a"
                            href={subItem.href}
